...
import { defaultVisualConfig, FORMAT_OPTIONS as FORMAT_CHOICES } from '../popup/steps/wizardConstants';
...
function ChartPreview({ type, colors }) {
...
export default function WidgetSettingsDialog({ open, item, onClose, onApply }) {
  ...
  const handleApply = () => {
    ...
    const nextSpec = {
      ...
      dataConfig: {
        ...(spec.dataConfig ?? {}),
        procedureName: dataConfig.procedureName,
      ...
      <DialogContent dividers>
        <Stack spacing={2}>
          ...
          <Box>
            ...
            <Typography variant="body2" fontFamily="monospace" fontWeight={700}>{dataConfig.procedureName || '-'}</Typography>
          </Box>
