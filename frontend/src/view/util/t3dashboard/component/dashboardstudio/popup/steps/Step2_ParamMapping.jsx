...
import { fetchDistinctValues, fetchDateRange } from './spDataApi';
...
function paramsForSource(ds) {
  if (ds.sourceType === 'TABLE') return tableWhereToParams(ds.tableConfig);
  return ds.params ?? [];
}
...
export default function Step2_ParamMapping({ draft, onDraftChange, compact = false, hideEmpty = false, mode = 'test' }) {
  ...
  if (parameterMappings.length === 0) {
    if (hideEmpty) return null;
    const allTable = dataSources.every((ds) => ds.sourceType === 'TABLE');
    const hasTable = dataSources.some((ds) => ds.sourceType === 'TABLE');
    return (
      <EmptyStateCard
        icon={
          allTable
        ...
        chipLabel={allTable ? 'TABLE' : hasTable ? 'TABLE 포함' : 'SP / VIEW'}
        chipColor={allTable ? '#f59e0b' : '#3b82f6'}
        title={allTable ? '매핑할 파라미터가 없습니다' : '입력값 없이 조회되는 소스입니다'}
        compact={compact}
      />
    );
  }
