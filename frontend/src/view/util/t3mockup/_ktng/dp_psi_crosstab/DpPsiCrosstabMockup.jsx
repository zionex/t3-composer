...
const ROWS = [
  // 거점 1
  { SALES_ORG: 'KT&G',  ACCOUNT: 'CU',          ITEM_LV3_CD: 'KING-RED', ITEM_NM: '레드 클래식',  MEASURE: 'P(생산)', vals: [12000, 11800, 12500, 13000, 12800, 13200, 13500, 13700, 14000], emphasize: false },
  { SALES_ORG: 'KT&G',  ACCOUNT: 'CU',          ITEM_LV3_CD: 'KING-RED', ITEM_NM: '레드 클래식',  MEASURE: 'S(판매)', vals: [11500, 11900, 12200, 12800, 12700, 13000, 13300, 13500, 13800], emphasize: false },
  { SALES_ORG: 'KT&G',  ACCOUNT: 'CU',          ITEM_LV3_CD: 'KING-RED', ITEM_NM: '레드 클래식',  MEASURE: 'I(재고)', vals: [ 2500,  2400,  2700,  2900,  3000,  3200,  3400,  3600,  3800], emphasize: true },
  { SALES_ORG: 'KT&G',  ACCOUNT: 'GS25',        ITEM_LV3_CD: 'KING-BLU', ITEM_NM: '블루 멘솔',     MEASURE: 'P(생산)', vals: [ 8000,  8200,  8400,  8500,  8700,  8900,  9000,  9100,  9200], emphasize: false },
  { SALES_ORG: 'KT&G',  ACCOUNT: 'GS25',        ITEM_LV3_CD: 'KING-BLU', ITEM_NM: '블루 멘솔',     MEASURE: 'S(판매)', vals: [ 7800,  8100,  8300,  8500,  8700,  8800,  8900,  9000,  9100], emphasize: false },
  { SALES_ORG: 'KT&G',  ACCOUNT: 'GS25',        ITEM_LV3_CD: 'KING-BLU', ITEM_NM: '블루 멘솔',     MEASURE: 'I(재고)', vals: [ 1800,  1900,  2000,  2000,  2000,  2100,  2200,  2300,  2400], emphasize: true },
  // illuvia
  { SALES_ORG: 'KT&G',  ACCOUNT: '편의점 전체', ITEM_LV3_CD: 'NGP-DEV',  ITEM_NM: 'illuvia DEV',   MEASURE: 'P(생산)', vals: [ 5000,  5300,  5500,  5800,  6100,  6300,  6500,  6700,  7000], emphasize: false },
  { SALES_ORG: 'KT&G',  ACCOUNT: '편의점 전체', ITEM_LV3_CD: 'NGP-DEV',  ITEM_NM: 'illuvia DEV',   MEASURE: 'S(판매)', vals: [ 4800,  5200,  5600,  5800,  6000,  6200,  6400,  6600,  6800], emphasize: false },
  { SALES_ORG: 'KT&G',  ACCOUNT: '편의점 전체', ITEM_LV3_CD: 'NGP-DEV',  ITEM_NM: 'illuvia DEV',   MEASURE: 'I(재고)', vals: [ 1200,  1300,  1200,  1200,  1300,  1400,  1500,  1600,  1800], emphasize: true },
  // 해외
  { SALES_ORG: 'KT&G GLOBAL', ACCOUNT: '인도',  ITEM_LV3_CD: 'EXPORT-K', ITEM_NM: '수출 KING',     MEASURE: 'P(생산)', vals: [15000, 15500, 16000, 16300, 16500, 17000, 17200, 17500, 18000], emphasize: false },
  { SALES_ORG: 'KT&G GLOBAL', ACCOUNT: '인도',  ITEM_LV3_CD: 'EXPORT-K', ITEM_NM: '수출 KING',     MEASURE: 'S(판매)', vals: [14500, 15300, 15800, 16100, 16400, 16700, 17000, 17300, 17800], emphasize: false },
  { SALES_ORG: 'KT&G GLOBAL', ACCOUNT: '인도',  ITEM_LV3_CD: 'EXPORT-K', ITEM_NM: '수출 KING',     MEASURE: 'I(재고)', vals: [ 3500,  3700,  3900,  4100,  4200,  4500,  4700,  4900,  5100], emphasize: true },
];
            ...
            <TableBody>
              {ROWS.map((r, i) => {
                const measureColor =
                  ...
                  'warning.main';
                const bgColor = r.emphasize ? 'warning.light' : (i % 6 < 3 ? 'transparent' : 'grey.50');
                return (
                  <TableRow key={i} sx={{ backgroundColor: bgColor }}>
                    {FIXED_COLS.map((c) => (
                      <TableCell key={c.name}
                        sx={{ textAlign: c.name === 'MEASURE' ? 'left' : 'left',
                              ...
                              backgroundColor: c.sticky ? (r.emphasize ? '#fff5e0' : '#fff') : undefined,
                              zIndex: c.sticky ? 1 : undefined }}>
                        {r[c.name]}
                      </TableCell>
                    ))}
                    {r.vals.map((v, j) => (
                      <TableCell key={j} sx={{ textAlign: 'right', fontFamily: 'monospace', color: r.emphasize ? 'warning.dark' : 'inherit' }}>
                        {fmtN(v)}
                      </TableCell>
                    ))}
                  </TableRow>
