 ...
 * VIEW/TABLE 데이터 소스 유형을 표시하는 공용 Chip.
 *
 * @param {{ value: 'VIEW'|'TABLE' }} props
 */
...
/** 선택된 소스 카드. active=true면 파란 테두리. */
function SelectedSourceCard({ ds, idx, active, onFocus, onRemove }) {
  return (
    <Paper
      variant="outlined"
      onClick={onFocus}
        ...
        {ds.sourceType === 'TABLE' && (
          <Box>
            ...
            <Stack direction="row" gap={0.5} flexWrap="wrap">
              ...
              <Chip
                size="small"
                label={ds.tableConfig?.topN ? `TOP ${ds.tableConfig.topN}` : '전체'}
...
export default function DirectSourcePanel({
  ...
  const allowedSourceTypes = useMemo(() => new Set(SOURCE_TYPES.map((type) => type.value)), []);

  const filteredCatalog = useMemo(() => {
    const q = searchText.trim().toLowerCase();
    return catalog.filter((entry) => {
      if (!allowedSourceTypes.has(entry.sourceType)) return false;
  ...
  }, [catalog, moduleFilter, typeFilter, searchText, allowedSourceTypes]);
  ...
  function addSource(entry) {
    ...
    const selected = {
      ...
      tableConfig: entry.sourceType === 'TABLE'
        ? { columns: [], whereConditions: [], orderBy: [] }
        : entry.tableConfig,
    };
  ...
  function addDirectInput() {
    ...
    setDirectInput({ sourceType: 'VIEW', sourceName: '', description: '' });
              ...
              >
                ...
                <AccordionDetails sx={{ px: 1, pt: 0, pb: 1 }}>
                  <Stack spacing={1}>
                    ...
                    <TextField
                      size="small" label="소스명" placeholder="VIEW/TABLE명"
