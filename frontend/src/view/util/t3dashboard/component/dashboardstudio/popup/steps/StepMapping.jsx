...
import { SourceTypeChip, ModuleChip } from '../../widgetbuilder/panels/DirectSourcePanel';
...
function SourceTargetPanel({ target, draft, onDraftChange, testResults, onRunTest }) {
  ...
  const defaultTab = ds?.sourceType === 'TABLE' ? 'table' : hasParams ? 'params' : 'preview';
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
      ...
      <Box sx={{ flex: 1, overflow: 'auto', minHeight: 0 }}>
        {activeTab === 'table' && ds.sourceType === 'TABLE' && (
          <Box sx={{ p: 1.5 }}>
...
function MergedTargetPanel({ target, draft, onDraftChange, testResults, onRunTest, onRunMerged }) {
  ...
  const tableSources = groupSources.filter((ds) => ds.sourceType === 'TABLE');
