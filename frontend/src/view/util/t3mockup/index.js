...
const T3SMART_SCM_ENTRIES = [
  ...
  // ─── T3SMARTSCM 운영 대시보드 11종 (2026-06 추가, wingui 운영 화면 1:1) ───
  { patternCode: 'dash_psi_balance',      patternLabel: 'PSI 균형 관제 — Sales/Supply/Inventory', layoutCategory: 'LAYOUT_DASHBOARD', category: 'dashboard', usage: 1,
    sourceMenuCd: 'UI_SA_PSI', sourceFilePath: 'view/snop/psibalance/PsiBalance.jsx',
    description: 'KPI 4종 + PSI Flow chart(Sales bar + Supply bar + Inventory line + Target/Safety + Today) + GAP 분석 + AI 분석',
    layers: [
      { key: 'kpi1',     title: 'KPI 1',     type: 'CHART', subtype: 'KPI_CARD',  position: { x: 0, y: 0, w: 3,  h: 3 } },
      { key: 'kpi2',     title: 'KPI 2',     type: 'CHART', subtype: 'KPI_CARD',  position: { x: 3, y: 0, w: 3,  h: 3 } },
      { key: 'kpi3',     title: 'KPI 3',     type: 'CHART', subtype: 'KPI_CARD',  position: { x: 6, y: 0, w: 3,  h: 3 } },
      { key: 'kpi4',     title: 'KPI 4',     type: 'CHART', subtype: 'KPI_CARD',  position: { x: 9, y: 0, w: 3,  h: 3 } },
      { key: 'psiFlow',  title: 'PSI Flow',  type: 'CHART', subtype: 'CHART_BAR', position: { x: 0, y: 3, w: 12, h: 5 } },
      { key: 'gapPanel', title: 'GAP 분석',  type: 'GRID',  subtype: 'GRID_BASE', position: { x: 0, y: 8, w: 7,  h: 4 } },
      { key: 'aiPanel',  title: 'AI 분석',   type: 'GRID',  subtype: 'GRID_BASE', position: { x: 7, y: 8, w: 5,  h: 4 } },
    ],
    component: lazy(() => import('./dash_psi_balance/DashPsiBalanceMockup')) },

  { patternCode: 'dash_sales_state',      patternLabel: '판매 현황 — SalesBoard',                 layoutCategory: 'LAYOUT_DASHBOARD', category: 'dashboard', usage: 1,
    sourceMenuCd: 'UI_SA_SALES_STATE', sourceFilePath: 'view/demandplan/dashboard/salesboard/SalesBoard.jsx',
    description: 'DashboardPanel 6 widgets — 연 목표/실적/Top 품목 KPI + 월별 추이 + 제품군 비중 + 거래처 TOP',
    layers: [
      { key: 'kpi1', title: '연 목표',  type: 'CHART', subtype: 'KPI_CARD',   position: { x: 0, y: 0, w: 4, h: 3 } },
      { key: 'kpi2', title: '연 실적',  type: 'CHART', subtype: 'KPI_CARD',   position: { x: 4, y: 0, w: 4, h: 3 } },
      { key: 'kpi3', title: 'Top 품목', type: 'CHART', subtype: 'KPI_CARD',   position: { x: 8, y: 0, w: 4, h: 3 } },
      { key: 'trend', title: '월별 추이', type: 'CHART', subtype: 'CHART_LINE', position: { x: 0, y: 3, w: 6, h: 5 } },
      { key: 'grpShare', title: '제품군 비중', type: 'CHART', subtype: 'CHART_BAR', position: { x: 6, y: 3, w: 6, h: 5 } },
      { key: 'accountGrid', title: '거래처 판매', type: 'GRID', subtype: 'GRID_BASE', position: { x: 0, y: 8, w: 12, h: 4 } },
    ],
    component: lazy(() => import('./dash_sales_state/DashSalesStateMockup')) },

  { patternCode: 'dash_inventory_state',  patternLabel: '재고 현황 — InventoryBoard',              layoutCategory: 'LAYOUT_DASHBOARD', category: 'dashboard', usage: 1,
    sourceMenuCd: 'UI_SA_INVENTORY_STATE', sourceFilePath: 'view/inventoryplan/analysis/inventoryboard/InventoryBoard.jsx',
    description: 'DashboardPanel 7 widgets — 추이 KPI 3종 + 결품/과잉 Alert + Slow Moving / Obsolete',
    layers: [
      { key: 'invTrend',   title: '재고 추이',  type: 'CHART', subtype: 'CHART_LINE', position: { x: 0, y: 0, w: 4, h: 3 } },
      { key: 'salesTrend', title: '판매 추이',  type: 'CHART', subtype: 'CHART_LINE', position: { x: 4, y: 0, w: 4, h: 3 } },
      { key: 'daysTrend',  title: '재고 일수',  type: 'CHART', subtype: 'CHART_BAR',  position: { x: 8, y: 0, w: 4, h: 3 } },
      { key: 'stockoutTop', title: '결품 Alert', type: 'GRID', subtype: 'GRID_BASE', position: { x: 0, y: 3, w: 6, h: 5 } },
      { key: 'excessTop',  title: '과잉 Alert', type: 'GRID', subtype: 'GRID_BASE', position: { x: 6, y: 3, w: 6, h: 5 } },
      { key: 'slowMoving', title: 'Slow Moving', type: 'GRID', subtype: 'GRID_BASE', position: { x: 0, y: 8, w: 6, h: 4 } },
      { key: 'obsolete',   title: 'Obsolete',  type: 'GRID', subtype: 'GRID_BASE', position: { x: 6, y: 8, w: 6, h: 4 } },
    ],
    component: lazy(() => import('./dash_inventory_state/DashInventoryStateMockup')) },

  { patternCode: 'dash_masterplan_state', patternLabel: '공급 계획 현황 — MasterPlanBoard',        layoutCategory: 'LAYOUT_DASHBOARD', category: 'dashboard', usage: 1,
    sourceMenuCd: 'UI_SA_MASTER_PLAN_STATE', sourceFilePath: 'view/masterplan/analysisreport/masterplanboard/MasterPlanBoard.jsx',
    description: 'DashboardPanel 7 widgets — BOH/총수요/총공급/충족량/충족률 KPI 5종 + 수요-공급 차트 + Capacity Load',
    layers: [
      { key: 'kpi1', title: 'BOH',     type: 'CHART', subtype: 'KPI_CARD', position: { x: 0,    y: 0, w: 2.4, h: 2 } },
      { key: 'kpi2', title: '총 수요', type: 'CHART', subtype: 'KPI_CARD', position: { x: 2.4,  y: 0, w: 2.4, h: 2 } },
      { key: 'kpi3', title: '총 공급', type: 'CHART', subtype: 'KPI_CARD', position: { x: 4.8,  y: 0, w: 2.4, h: 2 } },
      { key: 'kpi4', title: '충족량',  type: 'CHART', subtype: 'KPI_CARD', position: { x: 7.2,  y: 0, w: 2.4, h: 2 } },
      { key: 'kpi5', title: '충족률',  type: 'CHART', subtype: 'KPI_CARD', position: { x: 9.6,  y: 0, w: 2.4, h: 2 } },
      { key: 'demandSupply', title: '수요-공급 충족', type: 'CHART', subtype: 'CHART_BAR', position: { x: 0, y: 2, w: 12, h: 5 } },
      { key: 'capLoad',      title: 'Capacity Load', type: 'CHART', subtype: 'CHART_BAR', position: { x: 0, y: 7, w: 12, h: 5 } },
    ],
    component: lazy(() => import('./dash_masterplan_state/DashMasterPlanStateMockup')) },

  { patternCode: 'dash_supply_trend',     patternLabel: '거점별 공급/출하/재고 추이 — SupplyTrend', layoutCategory: 'LAYOUT_DASHBOARD', category: 'dashboard', usage: 1,
    sourceMenuCd: 'UI_MP_SUPPLY_TREND', sourceFilePath: 'view/masterplan/analysisreport/supplytrend/SupplyTrend.jsx',
    description: 'SearchArea (PlanScope + 거점 + 기간) + KPI 4종 + 추이 라인 + 거점 SKU 그리드',
    layers: [
      { key: 'filterBar', title: '조회 조건', type: 'GRID',  subtype: 'GRID_BASE',  position: { x: 0, y: 0, w: 12, h: 1 } },
      { key: 'kpi1', title: '총 재고',   type: 'CHART', subtype: 'KPI_CARD',   position: { x: 0, y: 1, w: 3, h: 2 } },
      { key: 'kpi2', title: '총 출하',   type: 'CHART', subtype: 'KPI_CARD',   position: { x: 3, y: 1, w: 3, h: 2 } },
      { key: 'kpi3', title: '총 공급',   type: 'CHART', subtype: 'KPI_CARD',   position: { x: 6, y: 1, w: 3, h: 2 } },
      { key: 'kpi4', title: '회전율',    type: 'CHART', subtype: 'KPI_CARD',   position: { x: 9, y: 1, w: 3, h: 2 } },
      { key: 'trendChart', title: '추이 차트', type: 'CHART', subtype: 'CHART_LINE', position: { x: 0, y: 3, w: 12, h: 5 } },
      { key: 'skuGrid',    title: 'SKU 현황',  type: 'GRID',  subtype: 'GRID_BASE',  position: { x: 0, y: 8, w: 12, h: 4 } },
    ],
    component: lazy(() => import('./dash_supply_trend/DashSupplyTrendMockup')) },

  { patternCode: 'dash_demandplan_state', patternLabel: '수요 계획 현황 — DemandPlanBoard',        layoutCategory: 'LAYOUT_DASHBOARD', category: 'dashboard', usage: 1,
    sourceMenuCd: 'UI_SA_DEMAND_PLAN_STATE', sourceFilePath: 'view/demandplan/dashboard/demandplanboard/DemandPlanBoard.jsx',
    description: 'DashboardPanel 9 widgets (3×3) — 진행/알림/정확도 + 상태/충족/진척 + 분포/예측/팀 정확도',
    layers: [
      { key: 'w1', title: '계획 진행',  type: 'GRID',  subtype: 'GRID_BASE',   position: { x: 0, y: 0, w: 4, h: 4 } },
      { key: 'w2', title: '알림',       type: 'GRID',  subtype: 'GRID_BASE',   position: { x: 4, y: 0, w: 4, h: 4 } },
      { key: 'w3', title: '정확도 KPI', type: 'CHART', subtype: 'KPI_CARD',    position: { x: 8, y: 0, w: 4, h: 4 } },
      { key: 'w4', title: '계획 상태',  type: 'CHART', subtype: 'CHART_BAR',   position: { x: 0, y: 4, w: 4, h: 4 } },
      { key: 'w5', title: '공급 충족',  type: 'CHART', subtype: 'CHART_BAR',   position: { x: 4, y: 4, w: 4, h: 4 } },
      { key: 'w6', title: '판매 진척',  type: 'CHART', subtype: 'CHART_PIE',   position: { x: 8, y: 4, w: 4, h: 4 } },
      { key: 'w7', title: '판매 분포',  type: 'CHART', subtype: 'CHART_PIE',   position: { x: 0, y: 8, w: 4, h: 4 } },
      { key: 'w8', title: '예측 대비 계획', type: 'CHART', subtype: 'CHART_BAR', position: { x: 4, y: 8, w: 4, h: 4 } },
      { key: 'w9', title: '팀별 정확도', type: 'CHART', subtype: 'CHART_BAR',  position: { x: 8, y: 8, w: 4, h: 4 } },
    ],
    component: lazy(() => import('./dash_demandplan_state/DashDemandPlanStateMockup')) },

  { patternCode: 'dash_location_inventory_state', patternLabel: '거점 재고 현황 — InventoryLocationBoard', layoutCategory: 'LAYOUT_DASHBOARD', category: 'dashboard', usage: 1,
    sourceMenuCd: 'UI_SA_LOCATION_INVENTORY_STATE', sourceFilePath: 'view/inventoryplan/analysis/inventorylocationboard/InventoryLocationBoard.jsx',
    description: 'DashboardPanel 5 widgets — 총수요/현재고/결품위험/잠재손실 KPI + 거점별 재고 현황 큰 그리드',
    layers: [
      { key: 'kpi1', title: '총 수요',     type: 'CHART', subtype: 'KPI_CARD',  position: { x: 0, y: 0, w: 3, h: 3 } },
      { key: 'kpi2', title: '현재 재고',   type: 'CHART', subtype: 'KPI_CARD',  position: { x: 3, y: 0, w: 3, h: 3 } },
      { key: 'kpi3', title: '결품 위험',   type: 'CHART', subtype: 'KPI_CARD',  position: { x: 6, y: 0, w: 3, h: 3 } },
      { key: 'kpi4', title: '잠재 손실',   type: 'CHART', subtype: 'KPI_CARD',  position: { x: 9, y: 0, w: 3, h: 3 } },
      { key: 'locatGrid', title: '거점 재고 현황', type: 'GRID', subtype: 'GRID_BASE', position: { x: 0, y: 3, w: 12, h: 9 } },
    ],
    component: lazy(() => import('./dash_location_inventory_state/DashLocationInventoryStateMockup')) },

  { patternCode: 'dash_plan_against_prfm', patternLabel: '계획 대비 실적 — PlanAgainstPrfm',      layoutCategory: 'LAYOUT_DASHBOARD', category: 'dashboard', usage: 1,
    sourceMenuCd: 'UI_FP_PLAN_AGAINST_PRFM', sourceFilePath: 'view/factoryplan/dashboard/planagainstprfmdashboard/PlanAgainstPrfmDashboard.jsx',
    description: 'DashboardPanel 6 widgets — 계획/실적 stack + 문제 분포 pie + 문제 추이 stack + 그리드/리스트/영향',
    layers: [
      { key: 'planStack',   title: '계획 stack',  type: 'CHART', subtype: 'CHART_BAR', position: { x: 0, y: 0, w: 4, h: 6 } },
      { key: 'prbmPie',     title: '문제 pie',    type: 'CHART', subtype: 'CHART_PIE', position: { x: 4, y: 0, w: 2, h: 6 } },
      { key: 'prbmStack',   title: '문제 stack',  type: 'CHART', subtype: 'CHART_BAR', position: { x: 6, y: 0, w: 6, h: 6 } },
      { key: 'planGrid',    title: '실적 grid',   type: 'GRID',  subtype: 'GRID_BASE', position: { x: 0, y: 6, w: 4, h: 6 } },
      { key: 'prbmGrid',    title: '문제 grid',   type: 'GRID',  subtype: 'GRID_BASE', position: { x: 4, y: 6, w: 3, h: 6 } },
      { key: 'impactStack', title: '영향 stack',  type: 'CHART', subtype: 'CHART_BAR', position: { x: 7, y: 6, w: 5, h: 6 } },
    ],
    component: lazy(() => import('./dash_plan_against_prfm/DashPlanAgainstPrfmMockup')) },

  { patternCode: 'dash_plan_against_prfm_detail', patternLabel: '계획 대비 실적 상세 — Detail',   layoutCategory: 'LAYOUT_DASHBOARD', category: 'dashboard', usage: 1,
    sourceMenuCd: 'UI_FP_PLAN_AGAINST_PRFM_DETAIL', sourceFilePath: 'view/factoryplan/dashboard/planagainstprfmdetaildashboard/PlanAgainstPrfmDetailDashboard.jsx',
    description: 'SearchArea (ActualSearchCondition + Bucket) + 계획/실적 일별 상세 큰 그리드 1개',
    layers: [
      { key: 'filterBar', title: '조회 조건', type: 'GRID', subtype: 'GRID_BASE', position: { x: 0, y: 0, w: 12, h: 1 } },
      { key: 'detailGrid', title: '상세 그리드', type: 'GRID', subtype: 'GRID_BASE', position: { x: 0, y: 1, w: 12, h: 11 } },
    ],
    component: lazy(() => import('./dash_plan_against_prfm_detail/DashPlanAgainstPrfmDetailMockup')) },

  { patternCode: 'dash_salesplan_state_detail', patternLabel: '판매계획 대비 실적 상세 — SalesPlanStateDetail', layoutCategory: 'LAYOUT_DASHBOARD', category: 'dashboard', usage: 1,
    sourceMenuCd: 'UI_SA_SALES_PLAN_STATE_DETAIL', sourceFilePath: 'view/snop/mdb/salesplanstatedetail/SalesPlanStateDetail.jsx',
    description: 'DashboardPanel 2 widgets — 주차별 BP/Plan/Actual 막대 + 팀×품목 비교 그리드',
    layers: [
      { key: 'barChart', title: '주차별 차트', type: 'CHART', subtype: 'CHART_BAR', position: { x: 0, y: 0, w: 12, h: 6 } },
      { key: 'compareGrid', title: '비교 그리드', type: 'GRID', subtype: 'GRID_BASE', position: { x: 0, y: 6, w: 12, h: 6 } },
    ],
    component: lazy(() => import('./dash_salesplan_state_detail/DashSalesPlanStateDetailMockup')) },

  { patternCode: 'dash_sales_mp_plan',    patternLabel: 'RTF 충족율 — SalesMPPlan',                layoutCategory: 'LAYOUT_DASHBOARD', category: 'dashboard', usage: 1,
    sourceMenuCd: 'UI_SA_SALES_MP_PLAN', sourceFilePath: 'view/snop/mdb/salesmpplan/SalesMPPlan.jsx',
    description: 'DashboardPanel 5 widgets — 월별 RTF Demand/Supply 막대 + 충족율 선 + 영업그룹/거래형태/제품군 RTF Pie + 분석 그리드',
    layers: [
      { key: 'rtfMain',    title: 'RTF 메인',     type: 'CHART', subtype: 'CHART_BAR', position: { x: 0, y: 0, w: 12, h: 5 } },
      { key: 'salesPie',   title: '영업그룹 pie', type: 'CHART', subtype: 'CHART_PIE', position: { x: 0, y: 5, w: 4,  h: 5 } },
      { key: 'accountPie', title: '거래형태 pie', type: 'CHART', subtype: 'CHART_PIE', position: { x: 4, y: 5, w: 4,  h: 5 } },
      { key: 'itemPie',    title: '제품군 pie',   type: 'CHART', subtype: 'CHART_PIE', position: { x: 8, y: 5, w: 4,  h: 5 } },
      { key: 'analysisGrid', title: 'RTF 분석',  type: 'GRID',  subtype: 'GRID_BASE', position: { x: 0, y: 10, w: 12, h: 4 } },
    ],
    component: lazy(() => import('./dash_sales_mp_plan/DashSalesMpPlanMockup')) },
...
// ─────────────────────────────────────────
// CJBO — CJ Bio (CJ제일제당 바이오, 아미노산/사료첨가제 사업) 운영 화면 → 17개 mockup
// 사업부: AN (Animal Nutrition) · TN (Total Nutrition) · BMS (Bio Material Solution)
// 액상/분말 (SOLID_GBN: L/S). 다통화 (KRW/USD).
// 17개 mockup 모두 CJBO 소스 .jsx 파일을 직접 분석해서 구조 그대로 재현 (2026-06).
// ─────────────────────────────────────────
const CJBO_ENTRIES = [
  // ── DP Service — CJBO 핵심 서비스 ───────────────────────────────────
  { patternCode: 'cjbo_cm_tariff_mgmt',     patternLabel: 'CJBO — Tariff 관리 등록 (CmTariffMgmt)', layoutCategory: 'LAYOUT_SINGLE', category: 'domain',
    usage: 1, description: 'UI_CM_TARIFF_MGMT. 28컬럼(13 hidden) + AMT_TITLE/QTY_TITLE 그룹헤더. styleCallback: TARIFF_GBN=S 면 TO_DT 잠금. POST demandplan/cmtariffmgmt/q1,s1,d1.',
    layers: [
      { key: 'mainGrid', title: '그리드', type: 'GRID', subtype: 'GRID_BASE', position: { x: 0, y: 0, w: 12, h: 12 } },
    ],
    component: lazy(() => import('./_cjbo/cm_tariff_mgmt/CjboCmTariffMgmtMockup')) },
  { patternCode: 'cjbo_dp_sku_ratio',       patternLabel: 'CJBO — SKU 비율 관리 (DpItemRatio / DpItemRatioOP)', layoutCategory: 'LAYOUT_SINGLE', category: 'domain',
    usage: 2, description: 'UI_DP_ITEM_RATIO (TP, 2-tab) + UI_DP_ITEM_RATIO_OP (OP, 3-tab + 1M/3M/6M/12M 실적반영). *_MON 변형은 소스 없음.',
    layers: [
      { key: 'mainGrid', title: '그리드', type: 'GRID', subtype: 'GRID_CROSSTAB', position: { x: 0, y: 0, w: 12, h: 12 } },
    ],
    component: lazy(() => import('./_cjbo/dp_sku_ratio/CjboDpSkuRatioMockup')) },
  { patternCode: 'cjbo_dp_pm_account',      patternLabel: 'CJBO — PM 거래처-품목 등록 (DpPmAccount)', layoutCategory: 'LAYOUT_SINGLE', category: 'domain',
    usage: 1, description: 'UI_DP_PM_ACCOUNT. 20컬럼(11 hidden). PM_ADD_YN 으로 셀 잠금/편집 (styleCallback Back_White/Back_Yellow).',
    layers: [
      { key: 'mainGrid', title: '그리드', type: 'GRID', subtype: 'GRID_BASE', position: { x: 0, y: 0, w: 12, h: 12 } },
    ],
    component: lazy(() => import('./_cjbo/dp_pm_account/CjboDpPmAccountMockup')) },
  { patternCode: 'cjbo_dp_org_management',  patternLabel: 'CJBO — 조직 관리 3종 (DpOrgMap / DpOrgChg / DpEntryTpOrg)', layoutCategory: 'LAYOUT_SINGLE', category: 'domain',
    usage: 3, description: '① OrgMap: LEVEL1/2/3 + APS_LEVEL + 17 popups + ATTR_01~20 · ② OrgChg: PRE/CUR 4컬럼 · ③ EntryTpOrg: TreeGrid 계층 (SearchArea 없음)',
    layers: [
      { key: 'mainGrid', title: '그리드', type: 'GRID', subtype: 'GRID_BASE', position: { x: 0, y: 0, w: 12, h: 12 } },
    ],
    component: lazy(() => import('./_cjbo/dp_org_management/CjboDpOrgManagementMockup')) },
  { patternCode: 'cjbo_dp_entry_report',    patternLabel: 'CJBO — OP/TP 계획 조회 4종 (DpEntryReport*)', layoutCategory: 'LAYOUT_SINGLE', category: 'domain',
    usage: 4, description: '4-variant matrix (OP/TP × 담당/관리자). 동적 크로스탭. TP 는 EMP 컬럼 숨김. 담당 는 Approve. POST demandplan/dpentry{,scm,tp,scmtp}report/q2 → /q1.',
    layers: [
      { key: 'mainGrid', title: '그리드', type: 'GRID', subtype: 'GRID_CROSSTAB', position: { x: 0, y: 0, w: 12, h: 12 } },
    ],
    component: lazy(() => import('./_cjbo/dp_entry_report/CjboDpEntryReportMockup')) },
  { patternCode: 'cjbo_dp_plan_accuracy',   patternLabel: 'CJBO — 수요계획 정확도 (DpPlanAccuracy)', layoutCategory: 'LAYOUT_SINGLE', category: 'domain',
    usage: 1, description: 'UI_DP_PLAN_ACCURACY. 단일 BaseGrid + STATIC 4 group header (M_1_ACT/M_3/M_2/M_1) — runtime YYYY.MM 교체. 차트/탭 없음. BG/FG_COLOR 행 styleCallback.',
    layers: [
      { key: 'mainGrid', title: '그리드', type: 'GRID', subtype: 'GRID_BASE', position: { x: 0, y: 0, w: 12, h: 12 } },
    ],
    component: lazy(() => import('./_cjbo/dp_plan_accuracy/CjboDpPlanAccuracyMockup')) },
  { patternCode: 'cjbo_dp_ver_diff',        patternLabel: 'CJBO — 버전 비교 (DpVerDiff)', layoutCategory: 'LAYOUT_SINGLE', category: 'domain',
    usage: 1, description: 'UI_DP_VER_DIFF. 동적 크로스탭 (DATE × VER_1/VER_2/DIFF_2/CONT_2 × QTY/PRC/AMT). PopPersonalize 컬럼 개인화.',
    layers: [
      { key: 'mainGrid', title: '그리드', type: 'GRID', subtype: 'GRID_CROSSTAB', position: { x: 0, y: 0, w: 12, h: 12 } },
    ],
    component: lazy(() => import('./_cjbo/dp_ver_diff/CjboDpVerDiffMockup')) },

  // ── DP Operations — 판매실적 / 거점변경 ──────────────────────────────
  { patternCode: 'cjbo_dp_sales_act',       patternLabel: 'CJBO — 판매 실적 (DpSalesAct)', layoutCategory: 'LAYOUT_SINGLE', category: 'domain',
    usage: 1, description: 'UI_DP_SALES_ACT. 동적 크로스탭 (DATE × ACT/OSO/TOT × QTY/PRC/AMT). totalOnlyYn (BILL/OPEN) 필터.',
    layers: [
      { key: 'mainGrid', title: '그리드', type: 'GRID', subtype: 'GRID_CROSSTAB', position: { x: 0, y: 0, w: 12, h: 12 } },
    ],
    component: lazy(() => import('./_cjbo/dp_sales_act/CjboDpSalesActMockup')) },
  { patternCode: 'cjbo_dp_loc_change',      patternLabel: 'CJBO — 거점 변경 관리 (DpLocChange)', layoutCategory: 'LAYOUT_SINGLE', category: 'domain',
    usage: 1, description: 'UI_DP_LOC_CHANGE. 18컬럼(8 hidden). CHANGE_TYPE R/N + FROM/TO LOC popup. P_ID 있는 행은 잠금. DP_BTN_LOC_CHANGE 배치 버튼.',
    layers: [
      { key: 'mainGrid', title: '그리드', type: 'GRID', subtype: 'GRID_BASE', position: { x: 0, y: 0, w: 12, h: 12 } },
    ],
    component: lazy(() => import('./_cjbo/dp_loc_change/CjboDpLocChangeMockup')) },

  // ── Plan Workflow — OP/TP/BP 계획 워크플로 (BaseControlBoard + BaseProcessStatus + BaseEntry) ──────
  // 모두 thin wrapper → Base{ControlBoard,ProcessStatus,Entry} 공유. planTypeCode 만 다름.
  // ControlBoard 구조: 수직 Stepper(버전이력) + approvalSteps 테이블 + 3 cards (VER_INFO, CLOSE_SETTING, STEP_DETAIL)
  // ProcessStatus 구조: 가로 Stepper alternativeLabel + TreeGrid
  // Entry 구조: 40 DIMENSION_NN(hidden) + CATEGORY(Measure) + DATE iteration prefix='DATE_'
  // (이전 mockup 의 '엔진 실행 로그 패널' 은 잘못 — 결재/승인 워크플로 + Drawer + 3 cards 가 정답)
  { patternCode: 'cjbo_op_workflow',        patternLabel: 'CJBO — OP 월간 워크플로 (planTypeCode=DP_PLAN_MONTHLY)', layoutCategory: 'LAYOUT_CONTROLBOARD', category: 'controlboard',
    usage: 3, description: 'UI_DP_93/94/95. BaseControlBoard + BaseProcessStatus + BaseEntry (hasChart=false). 3-탭 통합 mockup.',
    layers: [
      { key: 'stepper',    title: '패널 1', type: 'CHART', subtype: 'STEPPER',  position: { x: 0, y: 0, w: 12, h: 3 } },
      { key: 'kpi1',       title: 'KPI 1',  type: 'CHART', subtype: 'KPI_CARD', position: { x: 0, y: 3, w: 3,  h: 2 } },
      { key: 'kpi2',       title: 'KPI 2',  type: 'CHART', subtype: 'KPI_CARD', position: { x: 3, y: 3, w: 3,  h: 2 } },
  ...
  { patternCode: 'cjbo_tp_workflow',        patternLabel: 'CJBO — TP 목표 워크플로 (planTypeCode=DP_PLAN_TARGET)', layoutCategory: 'LAYOUT_CONTROLBOARD', category: 'controlboard',
    usage: 3, description: 'UI_DT_93/94/95. BaseControlBoard + BaseProcessStatus + BaseEntry (hasChart=true). VER_INFO 에 DP_YEARPLAN_YN/RATIO_COPY_YN/SIMUL_YN 추가.',
    layers: [
      { key: 'stepper',    title: '패널 1', type: 'CHART', subtype: 'STEPPER',  position: { x: 0, y: 0, w: 12, h: 3 } },
      { key: 'kpi1',       title: 'KPI 1',  type: 'CHART', subtype: 'KPI_CARD', position: { x: 0, y: 3, w: 3,  h: 2 } },
      { key: 'kpi2',       title: 'KPI 2',  type: 'CHART', subtype: 'KPI_CARD', position: { x: 3, y: 3, w: 3,  h: 2 } },
  ...
  { patternCode: 'cjbo_bp_workflow',        patternLabel: 'CJBO — BP 연간 워크플로 (planTypeCode=DP_PLAN_YEARLY)', layoutCategory: 'LAYOUT_CONTROLBOARD', category: 'controlboard',
    usage: 3, description: 'UI_BP_93/94/95. BaseControlBoard + BaseProcessStatus + BaseEntry (hasChart=true).',
    layers: [
      { key: 'stepper',    title: '패널 1', type: 'CHART', subtype: 'STEPPER',  position: { x: 0, y: 0, w: 12, h: 3 } },
      { key: 'kpi1',       title: 'KPI 1',  type: 'CHART', subtype: 'KPI_CARD', position: { x: 0, y: 3, w: 3,  h: 2 } },
      { key: 'kpi2',       title: 'KPI 2',  type: 'CHART', subtype: 'KPI_CARD', position: { x: 3, y: 3, w: 3,  h: 2 } },
  ...
  // ── EntryNotify + EntryLog ───────────────────────────────────────────
  { patternCode: 'cjbo_dp_plan_confirm',    patternLabel: 'CJBO — 계획 점검 + 입력 로그 (EntryNotify + EntryLog)', layoutCategory: 'LAYOUT_SINGLE', category: 'domain',
    usage: 2, description: 'UI_DP_ENTRY_NOTIFY (NotifyTable + Chart.js + 동적 BaseGrid · DP_ALERT_RANGE palette) + UI_DP_ENTRY_LOG (15-col read-only, engine/dp/GetEntryLog). DpEntryConf/TPConf 파일은 CJBO 소스에 없음.',
    layers: [
      { key: 'mainGrid', title: '그리드', type: 'GRID', subtype: 'GRID_BASE', position: { x: 0, y: 0, w: 12, h: 12 } },
    ],
    component: lazy(() => import('./_cjbo/dp_plan_confirm/CjboDpPlanConfirmMockup')) },

  // ── Widgets (소스 그대로 — 각 위젯 정확한 render type) ───────────────
  { patternCode: 'cjbo_widget_top_sales_map', patternLabel: 'CJBO — Top Sales 위젯 3종 (DpTopSalesAccount/Item/ItemGrp)', layoutCategory: 'LAYOUT_SINGLE', category: 'dashboard',
    usage: 3, description: 'common/data + SP_UI_SA_SALES_DP + P_VIEW1. Account=Google Maps + 리스트 · Item=pill 리스트 + AMT/QTY 토글 · ItemGrp=Chart.js stacked line + W/M 토글.',
    layers: [
      { key: 'widget1', title: '위젯 1', type: 'CHART', subtype: 'DIAGRAM_FLO', position: { x: 0, y: 0, w: 12, h: 12 } },
    ],
    component: lazy(() => import('./_cjbo/widget_top_sales_map/CjboWidgetTopSalesMapMockup')) },
  { patternCode: 'cjbo_widget_dashboard_y',  patternLabel: 'CJBO — DP 연간 위젯 12종 대시보드', layoutCategory: 'LAYOUT_SINGLE', category: 'dashboard',
    usage: 12, description: 'DpPlanStatusY (5-dataset chart) · DpYearActualSales/TargetSales (KPI) · ForecastPlan · PlanProgress (Stepper) · PlanStatus · SalesAlerts (리스트) · SalesPlanDistribution (2 doughnut) · SalesProgress · SupplySufRate · TeamSalesPlan (Slider 리스트) · Accuracy (2 Slider).',
    layers: [
      { key: 'kpi1',     title: 'KPI 1',  type: 'CHART', subtype: 'KPI_CARD',   position: { x: 0,  y: 0, w: 3,  h: 3 } },
      { key: 'kpi2',     title: 'KPI 2',  type: 'CHART', subtype: 'KPI_CARD',   position: { x: 3,  y: 0, w: 3,  h: 3 } },
      { key: 'kpi3',     title: 'KPI 3',  type: 'CHART', subtype: 'KPI_CARD',   position: { x: 6,  y: 0, w: 3,  h: 3 } },
  ...
  // ── MP — SKU 관리 · SCM Demand ──────────────────────────────────────
  { patternCode: 'cjbo_mp_sku_management',  patternLabel: 'CJBO — MP SKU/Capa 관리 4종 (ST_01~04)', layoutCategory: 'LAYOUT_SINGLE', category: 'domain',
    usage: 4, description: 'UI_MP_ST_01 SubSkuPriority · UI_MP_ST_02 ItemRes (CJBO 커스텀 view/cjbo/, CJBO_RES_* 헤더 + PACK 조건편집) · UI_MP_ST_03 PeriodCap (lineItemCapa/lineCapa sub-tabs) · UI_MP_ST_04 SubSkuProportion (ESNT_SALE_RATE %).',
    layers: [
      { key: 'mainGrid', title: '그리드', type: 'GRID', subtype: 'GRID_BASE', position: { x: 0, y: 0, w: 12, h: 12 } },
    ],
