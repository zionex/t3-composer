...
import { fetchStoredProcedureParams } from './steps/spDataApi';
import { normalizeParameterMappings, paramsObjectFromMappings } from '../generic/widgetSpecAdapter';
import { ensureVisualShape } from '../dashboardbuilder/dialogs/WidgetSettingsDialog';
import DirectSourcePanel, { SourceTypeChip } from '../widgetbuilder/panels/DirectSourcePanel';
import dashboardConfig from '../dashboardConfig';

function mssqlParamToSpParam(row) {
  if (row?.paramName) {
    return {
      ...
      isView: row.isView ?? row.paramName.startsWith('P_VIEW'),
      lookup: row.lookup,
      staticOptions: row.staticOptions,
    };
  }

  const paramName = (row.PARAMETER_NAME ?? '').replace(/^@/, '');
  const raw = (row.DATA_TYPE ?? '').toLowerCase();
  let dataType = 'STRING';
  if (['int', 'bigint', 'smallint', 'tinyint'].includes(raw)) dataType = 'INTEGER';
  else if (['decimal', 'numeric', 'float', 'real', 'money', 'smallmoney'].includes(raw)) dataType = 'NUMBER';
  else if (['date', 'datetime', 'datetime2', 'smalldatetime'].includes(raw)) dataType = 'DATE';
  return { paramName, dataType, required: false, description: '', defaultValue: '', isView: paramName.startsWith('P_VIEW') };
}
...
export default function DirectConfigWizard({
  ...
  const fetchedParamKeysRef = useRef(new Set());
  ...
  useEffect(() => {
    draft.dataSources
      ...
      .forEach((ds) => {
        const paramKey = ds.id || ds.sourceName;
        if (fetchedParamKeysRef.current.has(paramKey)) return;
        fetchedParamKeysRef.current.add(paramKey);
        hydrateParams(ds);
      });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [draft.dataSources.map((ds) => `${ds.id}:${ds.sourceName}:${ds.params?.length ?? 0}:${Boolean(ds.needsParamFetch)}`).join('|')]);

  async function hydrateParams(ds) {
    let params = null;
    try {
      const rows = await fetchStoredProcedureParams(ds.sourceName);
      if (rows.length > 0) params = rows.map(mssqlParamToSpParam).filter((param) => param.paramName);
    } catch (_) {
      // keep existing params
    } finally {
      setDraft((prev) => {
        const dataSources = prev.dataSources.map((item) =>
          item.id !== ds.id ? item : { ...item, ...(params ? { params } : {}), needsParamFetch: false }
        );
        syncWidgetDraft({ dataSources });
  ...
  function handleFinish() {
    ...
    const finalWidget = {
      ...
      widgetOptions: {
        ...
        dataConfig: primaryDs?.sourceType === 'TABLE' ? {
          ...
          dataSourceId: primaryDs?.id,
          procedureName: primaryDs?.sourceName ?? '',
          viewName: '',
          params: paramsObjectFromMappings(parameterMappings, primaryDs?.id),
          fallbackData: primaryDs?.mockData ?? [],
          timeout: 0,
        },
      },
    };
  ...
  const stepValid = useMemo(() => {
    ...
    const visualTargets = getVisualTargets(draft);
    const hasAnyData = draft.dataSources.some((ds) => (testResults[ds.id]?.rows ?? []).length > 0)
      || visualTargets.some((target) =>
        target.kind === 'merged' &&
        ((testResults[target.id] ?? testResults[MERGED_DATA_SOURCE_ID])?.rows ?? []).length > 0
      );
  ...
  }, [widget, draft, testResults, skipSourceStep]);
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      {/* ── Step content ────────────────────────────────────────────────── */}
      <Box sx={{ flex: 1, overflow: isSourceStep || isTestStep || isMappingStep || isConfirmStep ? 'hidden' : 'auto', p: isSourceStep ? 0 : skipSourceStep ? 1.25 : 2 }}>
      ...
      <Box sx={{ px: 2, pt: 1.5, pb: 2, borderTop: '1px solid #e2e8f0' }}>
        <Stack direction="row" justifyContent={showBackBtn ? 'space-between' : 'flex-end'}>
          {showBackBtn && (
            ...
            >
              {activeStep === 0 && onBack ? 'AI 분석으로 돌아가기' : '이전'}
            </Button>
          )}
          {isLast ? (
            <Button
