...
const CONTROL_LABEL_SX = { fontSize: 11, fontWeight: 700, color: '#64748b', mb: 0.5, lineHeight: 1.2 };

function ColSelect({ label, value, columns, onChange, required, helperText }) {
  return (
    <Box>
      <Typography sx={CONTROL_LABEL_SX}>{label}{required ? ' *' : ''}</Typography>
      <TextField
        select
      ...
      {columns.map((col) => {
        ...
        return (
          <Stack key={col} direction="row" alignItems="center" spacing={1}>
            ...
            />
            {checked && (
              <TextField size="small" placeholder="표시명" sx={{ ...FIELD_SX, flex: 1 }}
    ...
    <Stack spacing={1.5} sx={{ height: '100%', minHeight: 0, overflow: 'hidden' }}>
      ...
      <Box sx={{ border: '1px solid #e5eaf2', borderRadius: '8px', p: 1.25, bgcolor: '#fff' }}>
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'minmax(240px, 420px) minmax(0, 1fr)' }, gap: 1, alignItems: 'center' }}>
          <Box>
            <Typography sx={CONTROL_LABEL_SX}>위젯명</Typography>
            <TextField
              size="small"
              sx={FIELD_SX}
              value={widgetTitle}
      ...
      <Box sx={{
        ...
        <Stack spacing={1.25} sx={{ minWidth: 0, height: '100%', minHeight: 0, overflow: 'hidden' }}>
          ...
          <Box sx={{
            ...
            <SectionCard
              ...
              contentSx={{ flex: 1, minHeight: 0, overflow: 'auto' }}
            >
        ...
        <SectionCard
          ...
          contentSx={{ p: 1, flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}
        >
          <Box sx={{
            ...
            height: { xs: type === 'table' ? 260 : 220, lg: '100%' },
