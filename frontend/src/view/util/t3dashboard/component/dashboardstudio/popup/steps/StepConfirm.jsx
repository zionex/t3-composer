...
function vcSummary(vc) {
  ...
  if (type === 'kpi') return vc.valueField ? `값: ${vc.valueField}` : '값 미설정';
  if (['bar', 'bar_stacked', 'bar_h', 'line', 'area'].includes(type)) {
    const parts = [
      vc.xField && `X: ${vc.xField}`,
      (vc.yFields?.length ?? 0) > 0 && `Y: ${vc.yFields.join(', ')}`,
    ].filter(Boolean);
    return parts.length ? parts.join(' / ') : '컬럼 미설정';
  }
  if (type === 'bar_line') {
    const parts = [
      vc.xField && `X: ${vc.xField}`,
      (vc.series?.length ?? 0) > 0 && `Series: ${vc.series.map((s) => displayField(s.field)).join(', ')}`,
    ].filter(Boolean);
    return parts.length ? parts.join(' / ') : '컬럼 미설정';
  }
  if (['pie', 'doughnut'].includes(type)) {
    const parts = [
      vc.labelField && `라벨: ${vc.labelField}`,
      vc.valueField && `값: ${vc.valueField}`,
    ].filter(Boolean);
    return parts.length ? parts.join(' / ') : '컬럼 미설정';
  }
  if (type === 'table') return vc.columns?.length ? `${vc.columns.length}개 컬럼` : '컬럼 미설정';
  return '-';
}
...
function sourceIdsForMapping(mapping) {
  if (Array.isArray(mapping?.sources) && mapping.sources.length > 0) {
    return mapping.sources.map(String);
  }
  if (Array.isArray(mapping?.sourceIds) && mapping.sourceIds.length > 0) {
    return mapping.sourceIds.map(String);
  }
  if (Array.isArray(mapping?.dataSourceIds) && mapping.dataSourceIds.length > 0) {
    return mapping.dataSourceIds.map(String);
  }
...
function mappingAppliesToSource(mapping, sourceId) {
  ...
  if (mappingSourceIds.length > 0) return mappingSourceIds.includes(String(sourceId));
  return false;
}
...
function SectionCard({ title, children }) {
  return (
    <Box sx={{ mt: 1, border: '1px solid #e5eaf2', borderRadius: '8px', bgcolor: '#fafbfc', p: 1.25 }}>
      <Typography sx={{ mb: 1, fontSize: 11, fontWeight: 800, color: '#64748b' }}>
        {title}
      </Typography>
      {children}
    </Box>
...
function PreviewWidget({ visualConfig, data }) {
  ...
  if (!type) {
    return (
      <Box sx={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Typography sx={{ fontSize: 12, color: '#94a3b8' }}>시각화 설정이 없습니다.</Typography>
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <Typography sx={{ fontSize: 12, color: '#94a3b8' }}>지원하지 않는 시각화 유형: {type}</Typography>
    </Box>
  );
}
...
function SourceParameterSummary({ sources, parameterMappings }) {
  const spSources = sources.filter((source) => source.sourceType === 'STORED_PROCEDURE');

  if (!spSources.length) {
    return (
      <Typography sx={{ fontSize: 11, color: '#94a3b8' }}>
        SP 소스가 없어 표시할 파라미터가 없습니다.
      </Typography>
    );
  }
        ...
        return (
          <Box key={source.id} sx={{ border: '1px solid #e2e8f0', borderRadius: '8px', bgcolor: '#fff', p: 1 }}>
            <Stack direction="row" alignItems="center" spacing={0.75} sx={{ mb: 0.75 }}>
              <Chip size="small" label="SP" sx={{ height: 18, fontSize: 10, bgcolor: '#dbeafe', color: '#2563eb', fontWeight: 800 }} />
            ...
            {params.length ? (
              <ChipList
                items={params.map((mapping) => `${mapping.paramName}: ${paramValue(mapping)}`)}
                emptyText="설정된 파라미터가 없습니다."
              />
            ) : (
              <Typography sx={{ fontSize: 11, color: '#94a3b8' }}>설정된 파라미터가 없습니다.</Typography>
            )}
          </Box>
        );
      })}
...
function TableConditionSummary({ sources }) {
  const tableSources = sources.filter((source) => source.sourceType === 'TABLE');

  if (!tableSources.length) {
    return (
      <Typography sx={{ fontSize: 11, color: '#94a3b8' }}>
        TABLE 소스가 없어 표시할 조건이 없습니다.
      </Typography>
    );
  }
  ...
  return (
    <Stack spacing={0.75}>
      {tableSources.map((source) => {
        const config = source.tableConfig ?? {};
        const whereConditions = (config.whereConditions ?? []).filter((condition) => condition.mappingType !== 'UNUSED');
        const orderBy = config.orderBy ?? [];
        return (
          <Box key={source.id} sx={{ border: '1px solid #e2e8f0', borderRadius: '8px', bgcolor: '#fff', p: 1 }}>
            <Stack direction="row" alignItems="center" spacing={0.75} sx={{ mb: 0.75 }}>
              <Chip size="small" label="TABLE" sx={{ height: 18, fontSize: 10, bgcolor: '#fff7ed', color: '#ea580c', fontWeight: 800 }} />
              <Typography sx={{ fontSize: 12, fontWeight: 900, color: '#1e293b' }} noWrap title={source.sourceName}>
                {source.sourceName}
              </Typography>
            </Stack>
            <Typography sx={{ mb: 0.5, fontSize: 10, fontWeight: 800, color: '#94a3b8' }}>WHERE</Typography>
            <ChipList items={whereConditions.map(whereText)} emptyText="WHERE 조건이 없습니다." />
            <Typography sx={{ mt: 0.9, mb: 0.5, fontSize: 10, fontWeight: 800, color: '#94a3b8' }}>ORDER BY</Typography>
            <ChipList items={orderBy.map(orderText)} emptyText="ORDER BY 조건이 없습니다." />
            {(config.columns?.length ?? 0) > 0 && (
              <>
                <Typography sx={{ mt: 0.9, mb: 0.5, fontSize: 10, fontWeight: 800, color: '#94a3b8' }}>COLUMNS</Typography>
                <ChipList items={config.columns} />
              </>
            )}
          </Box>
        );
...
function findMergeConfigForTarget(draft, target) {
  if (target?.kind !== 'merged') return null;

  const group = (draft?.mergeConfig?.mergeGroups ?? []).find((item) => item.id === target.id);
  if (group) {
    return {
      ...draft.mergeConfig,
      ...group,
      type: group.type ?? draft.mergeConfig?.type,
      relationships: group.relationships ?? [],
      sourceIds: group.sourceIds ?? target.sourceIds ?? [],
      baseSourceId: group.baseSourceId ?? group.sourceIds?.[0],
    };
  }

  return draft?.mergeConfig ?? null;
}
...
export default function StepConfirm({ draft, testResults = {} }) {
  ...
  const widgets = getVisualTargets(draft).map((target) => {
    const mergeConfig = findMergeConfigForTarget(draft, target);
